		...
		*/
		...
		//RCC->AHBENR |= 0x400000;1: I/O port F clock enabled
		//  RCC->AHBENR |= 0x40; // 1: CRC clock enabled
		...
		TIM3->PSC = 999; //0x1F3F; //79
		TIM3->ARR = 10;//0xFA; (1.25 originally)  //800Hz
    
		
		//	TIM3->DIER |= 0x1; //Bit 0 UIE: Update interrupt enable
		...
		// register to configure the output channels to PWM mode
		 ...
		 //TIM3->CR1 |= 0x1; //Bit 0 CEN: Counter enable
		 // TIM3->CR1= ((TIM3->CR1 &	0xFF8E) | 0x1);
		// TIM3->CR2|= 0x88; 
		...
		//Therefore, I added the code above.
		...
		TIM3->CCMR1 |=0x70;
		TIM3->CCMR1 |=0x7000;
		// or activate both at the same time: TIM3->CCMR1 |=0x7070;
		//	TIM3->CCMR1 &=0xEFFF;
		...
		TIM3->CCR1 =2; //ARR=5 x 0.2 = 1. or ARR=250 x 0.2 = 50.
		TIM3->CCR2 =2; //ARR=5 x 0.2 = 1.
	...
	// 
     ...
     //TIM3->CNT=0;
		...
		//	3.4 — Measuring PWM Output.
				...
				// TIM3->EGR |= 0x6;
					TIM2->EGR |= 0x1; //reset tim2
				//	TIM3->EGR |= 0x1; //reset tim3
...
//        }
    ...
    if (GPIOC->ODR | (0x40)) // if red is on, red LED (PC6)
    {
        ...
        HAL_Delay(460); //wait 460 ms so that program can catch the off signal
    }
